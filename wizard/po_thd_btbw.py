...
class POThdBTBWizard(models.TransientModel):
    ...
    def get_datas(self):
        ...
        sql = """SELECT (pt.name) as nama_barang, (sum(pol.product_qty)) as qty, (u.name) as satuan, (po.name) as no_po, (sp.name) as no_btb, (sum(sm.product_uom_qty)) as qty_btb, (sum(pol.product_qty)-sum(sm.product_uom_qty)) as sisa_po, (mig.name) as no_inquiry1, (hd.complete_name) as dept, (mig2.name) as no_inquiry2 
                ...
                GROUP BY mig.id, mig2.id, hd.id, pr.id, po.id, pt.name, u.id, sp.name
                ORDER BY mig.id, mig2.id
                """
        ...
        for res in result:
            ...
            contents.append({
                ...
                "7" : res['qty_btb'],
                "8" : res['sisa_po'],
                "9" : res['no_inquiry1'] or res['no_inquiry2'],
                "10": res['dept'],
