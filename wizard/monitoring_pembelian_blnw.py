...
class MonitoringPembelianWizard(models.TransientModel):
    ...
    def get_datas(self):
        ...
        # if (self.periode == 1) :
        sql = """SELECT (pr.name) as no_pr, (to_char(pr.date :: DATE, 'DD Mon YYYY')) as tgl_pr, (pog.name) as no_or, (pt.default_code) as no_item, (pt.name) as nama_item, (sum(prl.product_qty)) as req_qty, (u.name) as satuan, (po.name) as no_po, (to_char(po.date_order :: DATE, 'DD Mon YYYY')) as tgl_po, (rp.name) as supplier, (sum(pol.product_qty)) as order_qty, (pol.price_unit) as harga, (pol.price_tax) as ppn, (pol.price_total) as total, (sum(sm.product_uom_qty)) as qty_btb, (sp.name) as no_btb, (to_char(sp.date_done :: DATE, 'DD Mon YYYY')) as tgl_btb
                ...
                LEFT JOIN vit_product_request_line prl ON prl.product_request_id = pr.id
                ...
                GROUP BY aat.id, po.id, pog.id, prl.id, pr.id, pol.id, pt.id, u.id, rp.id, sm.id, sp.id
                ORDER BY pr.date
                """
        ...
        for sheet, header_content in datas.items():
            ...
            for data in contents[0] :
                ...
                if i == 0 :
                    ...
                    worksheet.write(row, 23, ' ', cell_format['content_right'])
                    worksheet.write(row, 24, ' ', cell_format['content_left'])
